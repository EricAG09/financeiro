...
- **Sempre `getUser()`**, nunca `getSession()`, para qualquer decisão. `getSession()`
  ...
  servidor de auth.
- A renovação de sessão acontece em `src/proxy.ts` a cada requisição.
- `src/backend/auth/guards.ts#requireUser` redireciona quem não tem sessão — é UX, e o
  comentário no arquivo diz isso explicitamente.
- Mensagens de erro nunca revelam se um e-mail existe na base
  (`src/backend/auth/errors.ts`). Enumeração de usuário é vazamento.
- Senha mínima de 12 caracteres em `src/backend/validation/src/backend/validation/schemas/auth.ts`.
...
1. `src/shared/config/env.ts` separa `publicEnv` de `serverEnv()`; a segunda lança erro se
   for lida no browser.
2. `src/backend/supabase/admin.ts` e `src/backend/ai/registry.ts` importam `server-only` — o build
   quebra se um Client Component os alcançar.
3. O ESLint bloqueia `@/src/backend/supabase/admin` e o símbolo `serverEnv` em
   `src/frontend/**`.
...
Os schemas em `src/backend/validation/schemas/` são a fronteira de confiança e rodam **nas duas pontas**.
...
- valores monetários: inteiro, positivo, com teto (`src/backend/validation/primitives.ts`);
...
A CSP é gerada por requisição em `src/proxy.ts`, com nonce e `strict-dynamic`. Em
...
- Resposta do modelo de IA é renderizada como texto puro
  (`src/frontend/components/assistant/assistant-message.tsx`). Conteúdo gerado por LLM é
  entrada não confiável como qualquer outra.
- Nenhum HTML de terceiro é interpolado em página.

## Erros

- O usuário vê mensagem genérica e acionável (`src/frontend/components/ui/error-state.tsx`).
- `app/error.tsx` registra apenas o `digest` do erro; stack trace fica no
  servidor.
- Erro de autenticação é traduzido por `src/backend/auth/errors.ts`, sem revelar detalhe
  interno.
